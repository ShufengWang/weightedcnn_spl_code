...
try
  ...
  if( strcmp(roidb.name,'unlabel') )
      for i = 1:length(imdb.image_ids)
        ...
        load([experiment_dir 'data/candidates/mi_ma_score.mat']);
        roidb.rois(i) = my_attach_proposals(regions.pos_boxes{i}, regions.neg_boxes{i}, mi, ma);
      end
  else 
      for i = 1:length(imdb.image_ids)
        tic_toc_print('roidb (%s): %d/%d\n', roidb.name, i, length(imdb.image_ids));
...
%%%%%%%%%%%%%%%%%
function rec = my_attach_proposals(pos_boxes, neg_boxes, mi, ma)
